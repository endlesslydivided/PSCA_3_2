...
class AbilityController
{
    ...
    {
        response.sendFile(__dirname.replace(__dirname.split('\\').pop(),"") + 'static/html/login.html');
    }

    getRegisterPage(request,response)
    {
        response.sendFile(__dirname.replace(__dirname.split('\\').pop(),"") + 'static/html/register.html');
    }
    ...
    getResoursePage(request,response)
    {
        ...
        {
            response.status(200).send(`Resource ${request.payload.id}-${request.payload.username}-${request.payload.role}`);
        }
        else 
        {
    ...
    refreshToken(request,response)
    {
        ...
        {
            jwt.verify(request.cookies.refreshToken, refreshKey, async (err, payload) => 
            {
                if (err) 
                {
